...
// SCRUM-723 — confirmación antes de POST /:id/confirm, mismo patrón de overlay
// bespoke que CreateClientModal (no hay Modal/Dialog compartido en este codebase).
export default function SaveQuoteConfirmModal({ onCancel, onConfirm, isPending }: Props) {
      ...
      <Card variant="modal" className="w-full max-w-md my-4">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">
            {t('ventasDiseno:quote.saveConfirmModal.title')}
          </h2>
        ...
        <div className="px-5 py-4">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            {t('ventasDiseno:quote.saveConfirmModal.body')}
          </p>
        </div>
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-slate-100 dark:border-slate-700">
          ...
          <Button onClick={onConfirm} disabled={isPending} loading={isPending}>
            {t('ventasDiseno:quote.saveConfirmModal.confirm')}
          </Button>
        </div>
      </Card>
    </div>
