 ...
 *  - liderservicios@test.com   (lider_servicios) — Aaron, ve TODO incl. "+ Nuevo ticket"
 *  - tecnicoservicios@test.com     (tecnico_servicios) — NO debe ver "+ Nuevo ticket"
 *  - garantiasservicios@test.com  (garantias_servicios) — NO debe ver "+ Nuevo ticket"
 *
...
test('9. Miguel (garantias_servicios) — mismo gate: NO debe ver "+ Nuevo ticket"', async ({ page }) => {
  await login(page, 'garantiasservicios@test.com')
  await gotoHome(page)
...
test('14. Management guarda un valor nuevo — panel y leyenda se actualizan de inmediato', async ({ page, request }) => {
  ...
  const token = await apiLogin(request, 'gerencia@test.com')
...
test('16. Gate de backend — PUT installation-goal responde 403 para un rol sin permiso', async ({ request }) => {
  const token = await apiLogin(request, 'liderservicios@test.com') // lider_servicios
