 ...
 *  - carlos@illuminations.com.pa (tecnico_servicios) — NO tiene ventas_diseno.read, confirmado
...
test('7. Producto inactivo — NUNCA aparece, ni en modo completo', async ({ page }) => {
  ...
  const psql = (sql: string) =>
    execSync(
      `${sshBase} "cd /var/www/backend && docker compose -f infra/docker-compose.qa.yml exec -T postgres psql -U atlanticerp -d atlanticerp -c \\"SET search_path TO illuminations_ventas_diseno; ${sql}\\""  < /dev/null`,
      { encoding: 'utf8' },
    )
