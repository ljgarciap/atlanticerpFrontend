 ...
 * (password = email): servicio@illuminations.com.pa (Aaron Leis, lider_servicios).
 */
...
test('1. SCRUM-779 — marcar herramienta Dañada pide detalle obligatorio y aparece en el Kardex con cantidad/saldo', async ({ page, request }) => {
  const loginRes = await request.post(`${BASE}/api/auth/login`, { data: { email: 'servicio@illuminations.com.pa', password: 'servicio@illuminations.com.pa' } })
