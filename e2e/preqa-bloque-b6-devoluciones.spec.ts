...
test.beforeAll(async () => {
  ...
  execSync(
    `docker compose -f /Users/lgarcia/Documents/GitHub/Softclass/Illumination/atlanticerp/atlanticerp-backend/infra/docker-compose.yml exec -T postgres psql -U atlanticerp -d atlanticerp -c "truncate table atlantic_bodega.customer_return_lines, atlantic_bodega.customer_returns restart identity cascade; delete from atlantic_bodega.inventory_movements where catalog_product_id in (4,5); delete from atlantic_bodega.product_warehouse_stock where catalog_product_id in (4,5);"`,
    { encoding: 'utf-8' },
  )
})
...
test.describe('SCRUM-479/480/481 — Confirmar recepción física / Rechazar (ciclo completo)', () => {
  ...
  test('efecto verificado en base de datos: stock sumado con cantidad REAL (no la solicitada) + credit_note_notified_at sellado', async () => {
    const result = await queryDb(`
      ...
      from atlantic_bodega.customer_returns cr
      join atlantic_bodega.customer_return_lines l on l.customer_return_id = cr.id
      order by cr.id;
    `)
    expect(result).toContain('esperando_nota_credito')
    expect(result).toContain(' t '.trim()) // received/notified = t (true)

    const stock = await queryDb(`select catalog_product_id, quantity from atlantic_bodega.product_warehouse_stock where warehouse_id = 6 and catalog_product_id in (4,5) order by catalog_product_id;`)
  ...
  test('efecto de rechazo verificado en base de datos: NO suma inventario, NO sella credit_note_notified_at', async () => {
    const stock = await queryDb(`select coalesce(sum(quantity),0) from atlantic_bodega.product_warehouse_stock where catalog_product_id = 5 and warehouse_id != 6;`)
    ...
    const rejected = await queryDb(`select credit_note_notified_at is null as notified_null, received_at is null as received_null from atlantic_bodega.customer_returns where contact_name = 'Para Rechazo Pre-QA';`)
...
test.describe('SCRUM-477 — Ver guía de entrega original: mismos productos/cantidades que Pedidos', () => {
  test('la guía reusa exactamente OrderDocumentService — cross-check contra los OrderItem reales', async () => {
    const items = await queryDb(`select reference_snapshot, qty_delivered from atlantic_bodega.order_items where order_id = (select id from atlantic_bodega.orders where order_number = 'VR-9001') order by id;`)
...
test.describe('RN2 (REQ-417) acumulado + mensaje específico del backend', () => {
  test('segunda devolución que excede el tope acumulado del mismo producto muestra el mensaje real del backend, no el genérico', async ({ page }) => {
    ...
    const accumulated = parseInt((await queryDb(
      `select coalesce(sum(qty_requested),0) from atlantic_bodega.customer_return_lines where order_item_id = ${itemId} and customer_return_id in (select id from atlantic_bodega.customer_returns where status != 'rechazada');`,
    )).trim(), 10)
